  ...
  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <Card label="COMPLETED" value={status.completed} color="var(--el-primary)" />
        ...
        <Card label="PROGRESS" value={pct} suffix="%" color="var(--el-primary)" />
      </div>

      {status.total > 0 && (
        <div className="mb-6">
          <div className="w-full h-2 bg-[var(--el-primary-11)] border border-[var(--el-primary-33)]">
            <div
              className="h-full bg-[var(--el-primary)] transition-all duration-500"
              style={{ width: `${pct}%` }}
            />
          </div>
        </div>
...
function Card({
  ...
  return (
    <div className="border border-[var(--el-primary-33)] p-3 text-center">
      <div className="text-2xl font-bold mb-1" style={{ color: color || "var(--el-primary-99)" }}>
        {value.toLocaleString()}{suffix || ""}
      </div>
      <div className="text-[10px] tracking-wider text-[var(--el-primary-66)]">{label}</div>
    </div>
  );
}
